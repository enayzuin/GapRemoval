...
(Get-Content $processorFile) -replace '(?s)public async Task<List<\s*string\s*>> CutVideo\(.*?\{.*?\}', @'
public async Task<List<string>> CutVideo(List<(double start, double end)> silentParts)
{
    var parts = new List<string>();
    var tempFolder = Path.Combine(Path.GetTempPath(), "gap_parts_" + Guid.NewGuid().ToString("N"));
